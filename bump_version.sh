...
function update_rsmeta_version() {
    spaces_pattern=$'[ \t]*'
    filename=$1
    version=$2
...
function update_packagejson_version() {
    spaces_pattern=$'[ \t]*'
    filename=$1
    version=$2

    sed -i .bak "s/\"version\":$spaces_pattern\"$spaces_pattern.*spaces_pattern\"/\"version\": \"$(escape_version $version)\"/g" $filename
    rm $filename.bak
    git add $filename
}
